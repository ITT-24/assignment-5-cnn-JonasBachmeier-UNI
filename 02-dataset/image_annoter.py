# modified code from Emma Reichert: https://github.com/ITT-24/assignment-5-cnn-EmmaSophieReichert/blob/master/02-dataset/image_label.py
import cv2
import numpy as np
import pyglet
...
input_path = os.path.join(current_dir,'unnamed.jpg')
...
if len(sys.argv) > 2:
    input_path = os.path.join(current_dir,sys.argv[1])
    poseType = sys.argv[2]
else:
    print("Not enough entries: Using ./result_image_name.jpg poseType")
...
edit_img = cv2.resize(frame, new_dimensions, interpolation=cv2.INTER_AREA)
width =int(width/2)
height = int(height/2)
#Setup Pyglet Window
WINDOW_WIDTH = width
    ...
    if button == mouse.LEFT:
        ...
        if len(point_list_x) == 2:
            ...
            if(height2 > width2): #portrait format
                ...
                edit_img = cv2.warpPerspective(frame,mat,(resolution_y, resolution_x)) #use final resulution here!!
            else: #landscape format
                destination = np.float32(np.array([[0, 0], [resolution_x, 0], [resolution_x, resolution_y], [0, resolution_y]]))
                mat = cv2.getPerspectiveTransform(point_array, destination)
                edit_img = cv2.warpPerspective(frame,mat,(resolution_x, resolution_y)) #use final resulution here!!
